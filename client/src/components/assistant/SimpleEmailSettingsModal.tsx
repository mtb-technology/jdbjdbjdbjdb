...
import { DEFAULT_SIMPLE_EMAIL_PROMPT } from "@/pages/follow-up-assistant";

// Available AI models - matching the server configuration
const AI_MODELS = {
  google: [
    { value: "gemini-3-pro-preview", label: "Gemini 3 Pro (High Thinking)" },
...
interface SimpleEmailSettingsModalProps {
  open: boolean;
...
export const SimpleEmailSettingsModal = memo(function SimpleEmailSettingsModal({
  open,
  onOpenChange,
  aiModel,
  setAiModel,
  ...
  const handleResetDefault = () => {
    setSystemPrompt(DEFAULT_SIMPLE_EMAIL_PROMPT);
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Simpele Email Instellingen</DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-6 py-4">
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <Label className="text-sm font-medium">Systeem Prompt (Simpele Email)</Label>
              <Button
            ...
            <p className="text-xs text-muted-foreground">
              Deze prompt bepaalt hoe de AI email threads analyseert en conceptantwoorden genereert.
            </p>
          </div>
        </div>
